...
##brief @~chinese 多路相机指定输出哪些路的图像, bit0为第一路，bit1为第二路等等 
##~english Multichannel cameras specify which channels of images to output, bit0 for channel0 bit1 for channel1 etc.*/
IMAGE_CHANNEL_MASK = 0
##brief @~chinese 针对U2BMT120C-GW机型，设置LED灯光亮度,范围0-50, 值越大灯光越亮
#~english For U2BMT120C-GW model, control the LED output light */
LED_LIGHT = 1
##brief @~chinese 是否开启宽动态功能 @~english Whether to enable WDR function */
ENABLE_WDR = 2
##brief @~chinese 设置和获取可调焦镜头调焦值 @~english Set and get focus value of adjustable focus lens */
CONTROL_FOCUS_VALUE = 3
##brief @~chinese 获取调焦值的最小值 @~english Get the minimum value of the focus value */
CONTROL_FOCUS_MIN_VALUE = 4
##brief @~chinese 获取调焦值的最大值 @~english Get the maximum value of the focus value */
CONTROL_FOCUS_MAX_VALUE = 5
##brief @~chinese 获取调焦值的默认值 @~english Get the default value of the focus value */
CONTROL_FOCUS_DEFAULT_VALUE = 6
...
def CameraGetImageBufferEx2(hCamera, pImageData, uTimes):
    ...
    err_code = _sdk.CameraGetImageBufferEx2(hCamera, pImageData, byref(pImgInfo), byref(c_puTimestamp), c_uint(uTimes))
	...
	#return Returns the first address of the RGB data buffer on success; otherwise returns NULL.
	#note does not need to call @link #CameraReleaseFrameHandle @endlink
	#CKSDK_API BYTE* __stdcall CameraGetImageBufferEx3(HANDLE hCamera, stImageInfo *pImgInfo, UINT64 *puTimeStamp, UINT uTimes);
def CameraGetImageBufferEx3(hCamera, uTimes):
    _sdk.CameraGetImageBufferEx3.restype = c_void_p
    pImgInfo = stImageInfo()
    c_puTimestamp = c_ulonglong()
    pImgData = _sdk.CameraGetImageBufferEx3(hCamera, byref(pImgInfo), byref(c_puTimestamp), c_uint(uTimes))
    return (c_char_p(pImgData), pImgInfo, c_puTimestamp.value) if pImgData else (None, None, 0)
...
def CameraGetImageTransferMode(hCamera):
    pTransferMode = c_int()
    err_code = _sdk.CameraGetImageTransferMode(hCamera, byref(pTransferMode))
...
def CameraRequestTransferImage(hCamera):
    err_code = _sdk.CameraRequestTransferImage(hCamera)
    return err_code
...
def CameraGigeSetTransPacketSize(hCamera, packetSize):
	...
	#@ingroup __CK_GIGE_PAGE__
	#@~chinese
	#@brief 设置GigE相机GVSP包延时间隔时间
	#@param[in] hCamera  相机的句柄，由@link #CameraInit @endlink函数获得
	#@param[out] pDelayUs GVSP包间隔时间，单位是微秒
	#@return @link #__CK_ERRCODE__ 状态码@endlink
	#@note GVSP包间隔时间的范围为0至1000微秒，默认值为0
	#@see CameraGigeSetPacketDelayTime
	#@~english
	#@brief Set the delay interval for GVSP packets of GigE camera
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@param[out] pDelayUs GVSP packet interval time
	#@return @link #__CK_ERRCODE__ Status code @endlink
	#@note GVSP packet interval time range is 0 to 1000 microseconds, the default value is 0
	#@see CameraGigeSetPacketDelayTime
	#CKSDK_API CameraSdkStatus __stdcall CameraGigeGetPacketDelayTime(HANDLE hCamera, UINT *pDelayUs);
def CameraGigeGetPacketDelayTime(hCamera):
    c_pDelayUs = c_int()
    err_code = _sdk.CameraGigeGetPacketDelayTime(hCamera, byref(c_pDelayUs))
    return err_code, c_pDelayUs.value
    
    ##
	#@ingroup __CK_GIGE_PAGE__
	#@~chinese
	#@brief 获取GigE相机GVSP包延时间隔时间
	#@param[in] hCamera  相机的句柄，由@link #CameraInit @endlink函数获得
	#@param[out] delayUs GVSP包间隔时间，单位是微秒
	#@return @link #__CK_ERRCODE__ 状态码@endlink
	#@note GVSP包间隔时间的范围为0至1000微秒，默认值为0
	#@see CameraGigeGetPacketDelayTime
	#@~english
	#@brief Get the delay interval for GVSP packets of GigE camera
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@param[out] delayUs GVSP packet interval time
	#@return @link #__CK_ERRCODE__ Status code @endlink
	#@note GVSP packet interval time range is 0 to 1000 microseconds, the default value is 0
	#@see CameraGigeGetPacketDelayTime
	#CKSDK_API CameraSdkStatus __stdcall CameraGigeSetPacketDelayTime(HANDLE hCamera, UINT delayUs);
def  CameraGigeSetPacketDelayTime(hCamera, delayUs):
    err_code = _sdk.CameraGigeSetPacketDelayTime(hCamera, c_uint(delayUs))
    return err_code
    
    
    ##
	#@ingroup __CK_RECORD_PAGE__
	#@~chinese
	#@brief 开始录像
	#@param[in] hCamera  相机的句柄，由@link #CameraInit @endlink函数获得
	#@param[in] pstrSavePath 保存录像文件的路径
	#@param[in] iFormat 录像文件的格式，0: avi文件
	#@param[in] iBitrate 录像文件码流大小，有效范围-1和1000000-10000000，值越大压缩后的图像质量越好，压缩率越小，文件越大，-1表示不进行压缩
	#@param[in] iFrameRate 录像文件的帧率
	#@return @link #__CK_ERRCODE__ 状态码@endlink
	#@note 现在暂时只支持AVI视频文件，保存原始图像数据，不支持压缩
	#@see CameraPushOneFrame CameraStopRecord
	#@~english
	#@brief start recording
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@param[in] pstrSavePath The path to save the video file
	#@param[in] iFormat The format of the video file, 0: avi file
	#@param[in] iBitrate video file stream size, valid range -1 and 1000000-10000000, the larger the value, the better the image quality after compression, the smaller the compression rate, the larger the file, -1 means no compression
	#@param[in] iFrameRate The frame rate of the video file
	#@return @link #__CK_ERRCODE__ Status code @endlink
	#@note currently only supports AVI video files, save original image data, does not support compression
	#@see CameraPushOneFrame CameraStopRecord
	#CKSDK_API CameraSdkStatus __stdcall CameraStartRecord(HANDLE hCamera, const char *pstrSavePath, INT iFormat, INT iBitrate, INT iFrameRate);
def CameraStartRecord(hCamera, pstrSavePath, iFormat, iBitrate, iFramerate):
    err_code = _sdk.CameraStartRecord(hCamera, _str_to_string_buffer(pstrSavePath), c_int(iFormat), c_int(iBitrate), c_int(iFramerate))
    return err_code
	...
	#@brief 写一帧图像数据到录像文件
	#@param[in] hCamera  相机的句柄，由@link #CameraInit @endlink函数获得
	#@param[in] pFrameBuffer 指针，图像数据地址
	#@param[in] pImgInfo 图像数据信息
	#@return @link #__CK_ERRCODE__ 状态码@endlink
	#@note 必须在调用CameraStartRecord函数后才调用这个函数
	#@see CameraStartRecord CameraStopRecord
	#@~english
	#@brief Write a frame of image data to the video file
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@param[in] pFrameBuffer pointer, image data address
	#@param[in] pImgInfo image data information
	#@return @link #__CK_ERRCODE__ Status code @endlink
	#@note This function must be called after the CameraStartRecord function is called
	#@see CameraStartRecord CameraStopRecord
	#CKSDK_API CameraSdkStatus __stdcall CameraPushOneFrame(HANDLE hCamera, const BYTE *pFrameBuffer, const stImageInfo *pImgInfo);
def CameraPushOneFrame(hCamera, pFrameBuffer, pImgInfo):
    err_code = _sdk.CameraPushOneFrame(hCamera, byref(pFrameBuffer), byref(pImgInfo))
    return err_code
	...
	#@return @link #__CK_ERRCODE__ 状态码@endlink
	#@note 必须在调用CameraStartRecord函数后才调用这个函数
	#@see CameraStartRecord CameraPushOneFrame
	#@~english
	#@brief end recording
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@return @link #__CK_ERRCODE__ Status code @endlink
	#@note This function must be called after the CameraStartRecord function is called
	#@see CameraStartRecord CameraPushOneFrame
	#CKSDK_API CameraSdkStatus __stdcall CameraStopRecord(HANDLE hCamera);
def CameraStopRecord(hCamera):
    err_code = _sdk.CameraStopRecord(hCamera)
    return err_code
    
    ##
	#@ingroup __CK_UPGRADE__
	#@brief @~chinese 相机升级回调函数定义 @~english Callback function definition for image capture 
	#typedef void(__stdcall* CAMERA_UPGRADE_PROC)(INT percent, LPVOID lpParam);
CAMERA_UPGRADE_PROC = CALLBACK_FUNC_TYPE(c_int, c_void_p)
	...
	#@brief 升级相机
	#@param[in] hCamera  相机的句柄，由@link #CameraInit @endlink函数获得
	#@param[in] pFilePath  升级文件路径
	#@param[in] callback   升级回调函数，通知当前升级的进度
	#@param[in] lpParam    升级回调函数的参数
	#@return @link #__CK_ERRCODE__ 状态码@endlink
	#@note 升级函数在升级完成或者升级出错时才会返回，所以会运行一段时间，升级时的进度会通过回调函数返回。
	#升级时不要调用其它函数对相机进行控制。升级时请勿断开相机和电源。升级完成后请断电重启相机。
	#@~english
	#@brief upgrade camera
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@param[in] pFilePath upgrade file path
	#@param[in] callback Upgrade callback function to notify the current upgrade progress
	#@param[in] lpParam upgrade callback function parameter
	#@return @link #__CK_ERRCODE__ Status code @endlink
	#@note The upgrade function will only return when the upgrade is complete or an error occurs, so it will run for a period of time, and the upgrade progress will be returned through the callback function.
	#Do not call other functions to control the camera during the upgrade. Do not disconnect the camera and power supply during the upgrade.
	#Please power off and restart the camera after the upgrade is complete.
	#CKSDK_API CameraSdkStatus __stdcall CameraUpgrade(HANDLE hCamera, const char* pFilePath, CAMERA_UPGRADE_PROC callback, LPVOID lpParam);
def CameraUpgrade(hCamera, pFilePath, callback, lpParam):
    err_code = _sdk.CameraUpgrade(hCamera, _str_to_string_buffer(pFilePath), callback, c_void_p(lpParam))
    return err_code
	...
	#@ingroup __CK_ADVANCED_SETTINGS__
	#@~chinese
	#@brief 设置整形参数，参数的类型由@link #emSdkValueType @endlink定义
	#@param[in] hCamera 相机的句柄，由@link #CameraInit @endlink函数获得
	#@param[in] type 设置的参数类型，参考@link #emSdkValueType @endlink
	#@param[in] value 设置的参数值
	#@note IMAGE_CHANNEL_MASK用来设置多通道相机输出哪些通道的图像数据,value的二进制的每个bit位设置为1代表输出这个通道的数据,
	# 例如bit0代表输出通道0图像,bit1代表输出通道1图像,bit2代表输出通道2图像,bit3代表输出通道3图像.另外可以将这些bit
	# 进行组合代表同时输出哪几个通道的图像：<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x1);  //只输出第0通道的图像数据<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x2);  //只输出第1通道的图像数据<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x3);  //同时输出第0通道和第1通道的图像数据<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x7);  //同时输出第0通道、第1通道和第2通道的图像数据<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0xf);  //同时输出4个通道的图像数据<br>
	#@~english
	#@brief sets the shaping parameters, the parameter type is defined by @link #emSdkValueType @endlink
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@param[in] type Set the parameter type, refer to @link #emSdkValueType @endlink
	#@param[in] value Set parameter value
	#@note IMAGE_CHANNEL_MASK is used to set the image data of which channels are output by the multi-channel camera. Each bit of the binary value of the value is set to 1 to represent the data of this channel.
	# For example, bit0 represents the image of output channel 0, bit1 represents the image of output channel 1, bit2 represents the image of output channel 2, and bit3 represents the image of output channel 3. In addition, these bits
	# The combination represents which channels of images are output at the same time:<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x1); //Only output the image data of channel 0<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x2); //Only output the image data of the 1st channel<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x3); //Output the image data of channel 0 and channel 1 at the same time<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0x7); //Simultaneously output the image data of the 0th channel, the 1st channel and the 2nd channel<br>
	# CameraSetIntValue(hCamera, IMAGE_CHANNEL_MASK, 0xf); //output 4 channels of image data at the same time<br>
	#CKSDK_API CameraSdkStatus __stdcall CameraSetIntValue(HANDLE hCamera, emSdkValueType type, INT value);
def CameraSetIntValue(hCamera, type, value):
    err_code = _sdk.CameraSetIntValue(hCamera, c_int(type), c_int(value))
    return err_code
	...
	#@ingroup __CK_ADVANCED_SETTINGS__
	#@~chinese
	#@brief 获取整形参数，参数的类型由@link #emSdkValueType @endlink定义
	#@param[in] hCamera 相机的句柄，由@link #CameraInit @endlink函数获得
	#@param[in] type 设置的参数类型，参考@link #emSdkValueType @endlink
	#@param[in] pValue 设置的参数值
	#@~english
	#@brief gets the shaping parameters, the parameter type is defined by @link #emSdkValueType @endlink
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#@param[in] type Set the parameter type, refer to @link #emSdkValueType @endlink
	#@param[in] pValue Set parameter value
	#CKSDK_API CameraSdkStatus __stdcall CameraGetIntValue(HANDLE hCamera, emSdkValueType type, INT *pValue);
def CameraGetIntValue(hCamera, type, pValue):
    c_pValue = c_int()
    err_code = _sdk.CameraGetIntValue(hCamera, c_int(type), byref(c_pValue))
    return err_code, c_pValue.value
	...
	#@brief reboot camera
	#@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	#CKSDK_API CameraSdkStatus __stdcall CameraReboot(HANDLE hCamera);
def CameraReboot(hCamera):
    err_code = _sdk.CameraReboot(hCamera)
    return err_code
